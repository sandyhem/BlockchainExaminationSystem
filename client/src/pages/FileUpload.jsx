...
const FileUpload = ({state,paper}) => {
  ...
  const uploadToPrivateIPFS = async () => {
    ...
    try {
      ...
      await state.contract.methods.uploadPaper(paper.paperId,response.data.IpfsHash).send({ from: state.account });
  ...
  return (
    <div className="container-fluid vh-100 my-1  d-flex flex-column align-items-center justify-content-center" style={{ backgroundColor: "#ADD8E6" }}>
        ...
        <div className="row">
          ...
          <div className="col-md-6">
            ...
            <div className="border border-secondary border-dashed text-center p-4 rounded bg-light">
              ...
              <button className="btn btn-success" onClick={() => document.getElementById("fileInput").click()}>
                Browse Files
              </button>
            ...
            <button className="btn btn-primary mt-3 w-100" onClick={uploadToPrivateIPFS} disabled={!file || isUploading}>
              Upload File
            </button>
          ...
          <div className="col-md-6">
            {[
              ...
              { id: "teacherAddress", label: "Teacher Address", value: state.account },
              // { id: "cid", label: "CID", value: cid },
            ].map((field) => (
              <div key={field.id} className="form-floating mb-3">
                <input type="text" id={field.id} className="form-control" value={field.value} readOnly />
                <label htmlFor={field.id} className="fw-bold">{field.label}</label>
                <button onClick={() => copyToClipboard(field.id)} className="btn btn-outline-secondary mt-2 w-100">
